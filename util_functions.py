...
def sample_neg_TF(net, test_ratio=0.1, TF_num=333, train_pos=None, test_pos=None, max_train_num=None):
    ...
    # sample negative links for train/test
    train_num, test_num = len(train_pos[0]), len(test_pos[0])
    #// to print the ones pair for train , test data
...
def extractLinks2subgraphs(Atrain, Atest, train_pos, train_neg, test_pos, test_neg, h=1, max_nodes_per_hop=None, train_node_information=None, test_node_information=None,sk="MI"):
    ...
    def helper(A, links, g_label, node_information,nam):
        g_list = []
        duaa=0
        # print("links[0] ",links[0],file=link_file)
        for i, j in tqdm(zip(links[0],links[1])):
            #/// i, j is G(i)=> G(j) 1 from gold in train_pos
            # print(" i and j ",i,j,file=link_file)
            print(" i and j ",i,j,file=Output_file)
            g, n_labels, n_features = subgraph_extraction_labeling((i, j), A, h, max_nodes_per_hop, node_information)
            max_n_label['value'] = max(max(n_labels), max_n_label['value'])
            ...
            # from pprint import pprint
            # with open('final .txt', 'wt') as out:
            #     for g in g_list:
            #     # g.printinfo()            
            #         pprint(vars(g), stream=out)


        return g_list
    print('Extract enclosed subgraph...')
    train_graphs = helper(Atrain, train_pos, 1, train_node_information,nam="train_pos") + helper(Atrain, train_neg, 0, train_node_information,nam="train_neg")
    print("==============================================================================================================")
    test_graphs = helper(Atest, test_pos, 1, test_node_information,nam="test_pos") + helper(Atest, test_neg, 0, test_node_information,nam="test_neg")
    print(max_n_label)
...
def subgraph_extraction_labeling(ind, A, h=1, max_nodes_per_hop=None, node_information=None):
    ...
    for dist in range(1, h+1):
        ...
        # print("$$$$$$$$$$$$$$$$$$$$$Begin$$$$$$$$$$",file=Output_file)
        # print(nodes,file=Output_file)
        # # pprint(nodes, stream=out)
        # print("++++++++++++++++++++++++++++++++++++++++++++++++++",file=Output_file)
        print("dist",dist,file=Output_file)
        nodes_dist += [dist] * len(fringe)
        print("* represnt 0 for TF and  1 for G",nodes_dist,file=Output_file)
    # move target nodes to top
    # print(" ind ",ind[0],file=Output_file)
    nodes.remove(ind[0])
    nodes.remove(ind[1])
    print("neighbors nodes ",nodes ,file=Output_file)

    nodes = [ind[0], ind[1]] + list(nodes) 
    # print("nodes before",nodes,file=Output_file)
    #the first two number in the list in the main branch TF=>G , then will be the neighbors
    ...
    # print("toooool",len(g),file=Output_file)
